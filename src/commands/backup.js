...
exports.run = async (database, message, args) => {
    ...
    if (![""].includes(message.author.id)) return message.react(config.emojis.red);

    let argüman = args[0];
    let kategori = args[1];
    let id = args[2];

    if (!argüman) return message.channel.send(nixEmbed.setDescription(`Lütfen gerekli argümanları doldurup tekrar deneyin. \nÖrnek: \`${config.prefix}${this.conf.name} <Create/Delete>\``)).then(e => e.delete({ timeout: 20000 }) && message.react(config.emojis.notr));

    if (argüman == "create" || argüman == "al" || argüman == "oluştur") {
        if (!kategori) return message.channel.send(nixEmbed.setDescription(`Lütfen gerekli argümanları doldurup tekrar deneyin. \nÖrnek: \`${config.prefix}${this.conf.name} ${argüman} <Channel/Role>\``)).then(e => e.delete({ timeout: 20000 }) && message.react(config.emojis.notr));
    ...
    if (argüman == "load" || argüman == "yükle" || argüman == "kur" || argüman == "oluştur") {
        ...
        if (kategori == "channel" || kategori == "kanal") {
            ...
            ChannelDatabase.findOne({ guildID: config.guildID, channelID: id }, async (err , channelData) => {
                ...
                await message.channel.send(nixEmbed.setDescription(`\`${channelData.name}\` adlı **kanal** yedeği kullanılarak **kanal** oluşturulacak. \nOnaylıyorsanız alttaki emojiye basın.`)).then(e => {
                    ...
                    collect.on("collect", async e => {
                        setTimeout(async function () {
                            ...
                            if (logChannel) logChannel.send(nixEmbed.setDescription(`${message.author} (\`${message.author.id}\`) tarafından \`${channelData.name}\` (\`${channelData.channelID}\`) **kanalının** yedeği kurulmaya başlandı.`))
                        }, 500);
                    })
                })
            })
        ...
        if (kategori == "role" || kategori == "rol") {
            ...
            RoleDatabase.findOne({ guildID: config.guildID, roleID: id }, async (err , roleData) => {
                ...
                await message.channel.send(nixEmbed.setDescription(`\`${roleData.name}\` adlı rolün yedeği kullanılarak rol oluşturulup, üyelere dağıtılacaktır. \nOnaylıyorsanız alttaki emojiye basın.`)).then(e => {
                    e.react(config.emojis.onay);
                    const onay = (reaction, member) => reaction.emoji.id === config.emojis.onay && member.id === message.author.id;
                    const collect = e.createReactionCollector(onay, { time: 60000 });
                    ...
                    collect.on("collect", async e => {
                        setTimeout(async function () {
                            ...
                            if (logChannel) logChannel.send(nixEmbed.setDescription(`${message.author} (\`${message.author.id}\`) tarafından \`${roleData.name}\` (\`${roleData.rolID}\`) **rolünün** yedeği kurulmaya başlandı. \nRol izinleri ayarlanıp bütün üyelere dağıtılacaktır.`))
                        }, 500);
                    })
                })
            })
